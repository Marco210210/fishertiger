export const LEGACY_RULES = Object.freeze({
  ...
  scoring: { goalkeeperConceded: 0, penaltyMissed: -3, penaltySaved: 3, cleanSheet: 0 },
...
export const normalizeRules = (input = {}) => {
  ...
  return {
    ...
    bench: {
      ...
      mode: ["Basic", "Strict", "Traditional", "None"].includes(bench.mode) ? bench.mode : LEGACY_RULES.bench.mode,
    },
    scoring: {
      ...LEGACY_RULES.scoring,
      ...scoring,
      goalkeeperConceded: Number(scoring.goalkeeperConceded ?? scoring.goalkeeper_conceded ?? scoring.portiere_gol_subiti ?? LEGACY_RULES.scoring.goalkeeperConceded),
      penaltyMissed: Number(scoring.penaltyMissed ?? scoring.penalty_missed ?? LEGACY_RULES.scoring.penaltyMissed),
      penaltySaved: Number(scoring.penaltySaved ?? scoring.penalty_saved ?? LEGACY_RULES.scoring.penaltySaved),
      cleanSheet: Number(scoring.cleanSheet ?? scoring.clean_sheet ?? LEGACY_RULES.scoring.cleanSheet),
    },
