...
export const rulesFor = (profile, data = {}) => {
  ...
  const profileRules = {
    ...
    scoring: {
      ...object(fallback.scoring),
      goalkeeperConceded: pick(scoring.goalkeeper_conceded_goal, object(fallback.scoring).goalkeeperConceded),
      penaltyMissed: pick(scoring.penalty_missed, object(fallback.scoring).penaltyMissed),
      penaltySaved: pick(scoring.penalty_saved, object(fallback.scoring).penaltySaved),
      cleanSheet: pick(scoring.clean_sheet, object(fallback.scoring).cleanSheet),
    },
