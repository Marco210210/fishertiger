...
const SAVE_PATHS = [
  ...
  ["roster_slots"],
  ["profile_id"], ["name"], ["credits"], ["auction"],
];
...
export const profileChangePolicy = (baseline = {}, draft = {}) => {
  ...
  const fields = [...datasetFields, ...saveFields];
  const action = datasetFields.length ? "regenerate_dataset" : fields.length ? "save" : "none";
  return { action, fields, datasetFields, saveFields, dirty: fields.length > 0 };
};
